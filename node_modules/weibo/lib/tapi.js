...
if (typeof require !== 'undefined') {
	...
	var TwitterAPI = require('./twitter').TwitterAPI;
	var TQQAPI = require('./tqq').TQQAPI;
	var TSOHUAPI = require('./tsohu').TSOHUAPI;
} else {
	var TSinaAPI = weibo.tsina.TSinaAPI;
	var TwitterAPI = weibo.twitter.TwitterAPI;
	var TQQAPI = weibo.tqq.TQQAPI;
	var TSOHUAPI = weibo.tsohu.TSOHUAPI;
}
...
Object.extend(module.exports, {
  ...
  api_dispatch: function(data) {
	...
	unread: function(data, callback, context) {
		return this.api_dispatch(data).unread(data, callback, context);
	},
	...
	update: function(data, callback, context) {
		if (!data.pic) {
			this.api_dispatch(data).update(data, callback, context);
			return;
		}
		// 包含图片
		var pic = data.pic;
		delete data.pic;
		this.api_dispatch(data).upload(data, pic, callback, context);
	},
